  ...
  private func handleCAGradientProperties(_ acGradient: NSDictionary) -> CAGradientLayer {
    ...
    for property in properties {
      if acGradient[property] != nil {
        ...
        case "colors":
          do {
            gradiant.colors = self.convertHexColorsToCGColors(
              (acGradient[property] as? [String?] ?? []))
            break
          }
        case "locations":
          do {
            gradiant.locations = self.handleLocationGradient(
              (acGradient[property] as? [Int?] ?? []))
            break
          }
        case "startPoint":
          do {
            gradiant.startPoint = self.handleCGPoints(
              (acGradient[property] as? NSDictionary), defaultPoints: CGPoint.init(x: 0, y: 0))
            break
          }
        case "endPoint":
          do {
            gradiant.endPoint = self.handleCGPoints(
              (acGradient[property] as? NSDictionary), defaultPoints: CGPoint.init(x: 1, y: 0))
            break
          }
        default:
          do {
